...
public class DateUtils {
     ...
     * 日期比较，如果s<=e 返回true 否则返回false
     */
    public static boolean compareDateC(String s, String e) {
        if (fomatDate(s) == null || fomatDate(e) == null) {
            return false;
        }
        return fomatDate(s).getTime() < fomatDate(e).getTime();
...
//		day = (endDateStr.getTime() - beginDateStr.getTime()) / (24 * 60 * 60 * 1000);
     ...
     * @throws ParseException 
     */
    public static Date getTimeByMinuteForDate(String date ,int minute) throws ParseException {
        Calendar calendar = Calendar.getInstance();
     ...
     * @param startDay  开始日期 例如:2018-03-10
     * @return
     */
     ...
     * 根据指定日期开始得到剩余日期的集合 格式为yyyy-MM-dd
     * @param startDay  开始日期 例如:2018-03-10
     * @param endDay  开始日期 例如:2019-03-10
     * @return
     */
    public static List<String> getDayList(String startDate,String endDate) {   	
        ...
        int endYear = Integer.valueOf(endDayArray[0]);
        int endMonth = Integer.valueOf(endDayArray[1]);
        int endDay = Integer.valueOf(endDayArray[2]);
        
        c.set(startYear, startMonth,startDay);
        endC.set(endYear, endMonth,endDay);
    ...
    /**
     * 根据指定月开始到截止月的集合 格式为yyyy-MM
     * @param startMonth  开始月 例如:2018-03
     * @param endMonth  结束月 例如:2019-03
     * @return
     */
    public static List<String> getMonthList(String startMonth,String endMonth) {   	
    	String[] dayArray = startMonth.split("-");    	
    	Calendar c = Calendar.getInstance();
    	
    	String[] endDayArray = endMonth.split("-");  
    	Calendar endC = Calendar.getInstance();
    	
        DateFormat df = new SimpleDateFormat("yyyy-MM");       
        //分割日期字符串 得到dayArray[0]为年   dayArray[1]为月    dayArray[2]为日
        int startYear = Integer.valueOf(dayArray[0]);
        int beginMonth = Integer.valueOf(dayArray[1]) - 1;
      
        
        int endYear = Integer.valueOf(endDayArray[0]);
        int lastMonth = Integer.valueOf(endDayArray[1]) - 1;

        
        c.set(startYear, beginMonth,1);
        endC.set(endYear, lastMonth,1);
