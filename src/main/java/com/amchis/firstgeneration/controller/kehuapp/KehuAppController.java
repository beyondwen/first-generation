...
import com.amchis.firstgeneration.mapper.kehuapp.xiazaigengxins.XiazaigengxinsMapper;
...
public class KehuAppController extends BaseApiService {
    ...
    @Autowired
    private XiazaigengxinsMapper xiazaigengxinsMapper;
    ...
    public ReponseVo zidongdenglu(HttpServletRequest request) {
        ...
        if (!StringUtils.isEmpty(Shujuriqi)) {
            List<Tijian5s> tijian5s = tijian5sMapper.selectByshujuriAndWaijian(Shujuriqi, biaoji);
            Zidongdenglu guanjia = yonghu2sMapper.HuoquGuanjia(biaoji);
            if (guanjia == null) {
                guanjia = new Zidongdenglu();
            }
            String guanjiagonghao = guanjia.getGuanjiagonghao();
            Yonghu2s yonghu2s = yonghu2sMapper.ChaxunYigeYonghu2s(biaoji);
            for (Tijian5s tijian5 : tijian5s) {
                if (!StringUtils.isEmpty(guanjiagonghao)) {
                    tijian5.setGuanjiacunzai("1");
                } else {
                    tijian5.setGuanjiacunzai("0");
                }
                tijian5.setJiaoxueUrl("http://www.iqiyi.com/w_19rzjqdzx9.html");
                if (yonghu2s != null && "0".equals(yonghu2s.getShenfenzheng()) && "0".equals(yonghu2s.getMingcheng())) {
                    tijian5.setWenjuancunzai("1");
                } else {
                    tijian5.setWenjuancunzai("0");
                }
                ...
                getTijian5s(tijian5, keHuYaoWu18s);
            }
    ...
    public ReponseVo bangdingguanjia(HttpServletRequest request, @Valid @RequestBody Bangdingguanjia bangdingguanjia, BindingResult bindingResult) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            String guanjiaGonghao = bangdingguanjia.getGuanjiaGonghao();
            if ("0".equals(guanjiaGonghao)) {
    ...
    private ReponseVo saveYaowu(@RequestBody @Valid Kehuyaowu18s kehuyaowu18s, String biaoji) {
        ...
        if (!StringUtils.isEmpty(kehuyaowu18s.getZaoYidao()) && !StringUtils.isEmpty(kehuyaowu18s.getZaoYidaoLiang())) {
            ...
            reponseVo = getResponseVo(kehuyaowu18s, biaoji);
        }
    ...
    public ReponseVo tijianshuju(HttpServletRequest request, @Valid @RequestBody Tijian5s tijian5s, BindingResult bindingResult) {
        String biaoji = (String) request.getAttribute("Biaoji");
        String shoujihao = (String) request.getAttribute("Shoujihao");
        if (bindingResult.hasErrors()) {
            FieldError fieldError = bindingResult.getFieldError();
            ...
            BangDingKongZhiQi15s bangDingKongZhiQi15s = bangDingKongZhiQi15sMapper.selectByPhone(shoujihao);
            if (bangDingKongZhiQi15s == null) {
                return setResult("0", "KEHU_WEIBANGDING_WIFI", "检测到您未绑定DNAMY，如果您已经购买了DNAMY，请联系您的健康管家绑定，否则DNAMY将无法下载生物波！");
            } else {
                ...
                if (tijian5s.getId() != null) {
                    int i = tijian5sMapper.updateByPrimaryKeySelective(tijian5s);
                    if (toDaoResult(i)) {
                        return setResult("1", "TIANJIA_RICHANG_CHENGGONG", "更新体检数据成功!");
                    } else {
                        return setResult("0", "RICHANG_LURU_SHIBAI", "体检数录入失败，请联系管理员！");
                    }
                } else {
                    tijian5s.setWaijian(GetTime.getGuid());
                    tijian5sMapper.insertSelective(tijian5s);
                    String wifiming = bangDingKongZhiQi15s.getWifiming();
                    Xiazaigengxins xiazaigengxins = xiazaigengxinsMapper.selectByXuliehao(wifiming);
                    if (xiazaigengxins != null) {
                        xiazaigengxins.setShengbo(1);
                        int i = xiazaigengxinsMapper.updateByPrimaryKeySelective(xiazaigengxins);
                        if (!toDaoResult(i)) {
                            return setResult("1", "XIAZAIGENGXI_ZHIDING_CHENGGONG", "更新标记成功!");
                        } else {
                            return setResult("0", "XIAZAIGENGXI_ZHIDING_SHIBAI", "下载更新标记未指定成功，无法下载生物波，请向您的健康管家咨询解决！");
                        }
                    } else {
                        return setResult("0", "WIFI_JIANCE_SHIBAI", "未检测到您的设备，无法下载生物波，请向您的健康管家咨询解决！");
                    }
                }
            }
        }
    ...
    public ReponseVo tijianiosshuju(HttpServletRequest request, @Valid @RequestBody Tijian5s tijian5s, BindingResult bindingResult) {
        String biaoji = (String) request.getAttribute("Biaoji");
        String shoujihao = (String) request.getAttribute("Shoujihao");
        if (bindingResult.hasErrors()) {
            FieldError fieldError = bindingResult.getFieldError();
    ...
    public ReponseVo huoquguanjia(HttpServletRequest request) {
        String biaoji = (String) request.getAttribute("Biaoji");
        Zidongdenglu guanjia = yonghu2sMapper.HuoquGuanjia(biaoji);
    ...
    public ReponseVo huixiantijian(HttpServletRequest request, @Valid @RequestBody ShujuRiqi shujuRiqi, BindingResult bindingResult) {
        ...
        if (bindingResult.hasErrors()) {
            ...
            List<Tijian5s> tijian5s = tijian5sMapper.selectByshujuriAndWaijian(shujuRiqi.getShujuRiqi(), biaoji);
            if (tijian5s != null) {
                return setResult("1", tijian5s);
            } else {
                return setResult("0", "JINRI_WEITIANXIE", "今日未填写");
    ...
    public ReponseVo huoqukehuyaowu(HttpServletRequest request) {
        String biaoji = (String) request.getAttribute("Biaoji");
        String shoujihao = (String) request.getAttribute("Shoujihao");
        List<Kehuyaowu18> kehuyaowu18s = keHuYaoWu18sMapper.ChaxunAppKehuyaowu4kehuyaowu(biaoji);
    ...
    public ReponseVo huoquyaowuqushi(HttpServletRequest request) {
        String biaoji = (String) request.getAttribute("Biaoji");
        ...
        if (kehuyaowu18s != null && kehuyaowu18s.size() > 0) {
            for (Kehuyaowu18s kehuyaowu18 : kehuyaowu18s) {
                ...
                if ("ZAO".equals(kehuyaowu18.getQujian())) {
                    if ("KOUFUYAO1".equals(kehuyaowu18.getLeixing())) {
                        Yaowuqushishuju yaowuqushishuju = new Yaowuqushishuju();
                    ...
                    if ("KOUFUYAO2".equals(kehuyaowu18.getLeixing())) {
                        Yaowuqushishuju yaowuqushishuju = new Yaowuqushishuju();
                    ...
                    if ("KOUFUYAO3".equals(kehuyaowu18.getLeixing())) {
                        Yaowuqushishuju yaowuqushishuju = new Yaowuqushishuju();
                        yaowuqushishuju.setLeixing("ZAOKOUFUYAO2");
                        yaowuqushishuju.setShujushijian(kehuyaowu18.getShujuriqi());
                        yaowuqushishuju.setShujuzhi(kehuyaowu18.getJiliang());
